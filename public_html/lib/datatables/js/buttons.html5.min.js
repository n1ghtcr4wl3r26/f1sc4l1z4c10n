        ...
        action: function(a, b, e, c) {
            ...
            e = function(a) {
                for (var b = [], c = 0, d = a.length; c <
                    ...
                    b.push("number" === typeof a[c] || a[c].match && g.trim(a[c]).match(/^-?\d+(\.\d+)?$/) ? '<c t="n"><v>' + a[c] + "</v></c>" : '<c t="inlineStr"><is><t>' + (!a[c].replace ? a[c] : a[c].replace(/&(?!amp;)/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/[\x00-\x09\x0B\x0C\x0E-\x1F\x7F-\x9F]/g, "")) + "</t></is></c>")
                }
