package anb.entidades;

public class Declaracion {

    private String tramite;
    private String gestion;
    private String aduana;
    private String numero;
    private String fechaRegistro;
    private String fechaValidacion;
    private String operador;
    private String operadorNombre;
    private String declarante;
    private String declaranteNombre;
    private String proveedor;
    private String origen;
    private String estado;
    private String codigo;
    private String fechaPase;
    private String sequencia;
    private String verifitem;
    private String canal;

    private String key_year;
    private String key_cuo;
    private String key_dec;
    private String key_nber;
    private String sad_reg_year;
    private String sad_reg_serial;
    private String sad_reg_nber;
    private String sad_consignee;
    private String sad_reg_date;
    private String sad_itm_total;

    private String ficha;
    private String fechaLevante;
    private String items;
    private String patron;

    private String param;

    public void setGestion(String gestion) {
        this.gestion = gestion;
    }

    public String getGestion() {
        return gestion;
    }

    public void setAduana(String aduana) {
        this.aduana = aduana;
    }

    public String getAduana() {
        return aduana;
    }

    public void setNumero(String numero) {
        this.numero = numero;
    }

    public String getNumero() {
        return numero;
    }

    public void setDeclarante(String declarante) {
        this.declarante = declarante;
    }

    public String getDeclarante() {
        return declarante;
    }

    public void setOperador(String operador) {
        this.operador = operador;
    }

    public String getOperador() {
        return operador;
    }

    public void setOrigen(String origen) {
        this.origen = origen;
    }

    public String getOrigen() {
        return origen;
    }

    public void setEstado(String estado) {
        this.estado = estado;
    }

    public String getEstado() {
        return estado;
    }

    public void setFechaRegistro(String fechaRegistro) {
        this.fechaRegistro = fechaRegistro;
    }

    public String getFechaRegistro() {
        return fechaRegistro;
    }

    public void setTramite(String tramite) {
        this.tramite = tramite;
    }

    public String getTramite() {
        return tramite;
    }

    public void setFechaValidacion(String fechaValidacion) {
        this.fechaValidacion = fechaValidacion;
    }

    public String getFechaValidacion() {
        return fechaValidacion;
    }

    public void setOperadorNombre(String operadorNombre) {
        this.operadorNombre = operadorNombre;
    }

    public String getOperadorNombre() {
        return operadorNombre;
    }

    public void setDeclaranteNombre(String declaranteNombre) {
        this.declaranteNombre = declaranteNombre;
    }

    public String getDeclaranteNombre() {
        return declaranteNombre;
    }

    public void setProveedor(String proveedor) {
        this.proveedor = proveedor;
    }

    public String getProveedor() {
        return proveedor;
    }

    public void setCodigo(String codigo) {
        this.codigo = codigo;
    }

    public String getCodigo() {
        return codigo;
    }

    public void setFechaPase(String fechaPase) {
        this.fechaPase = fechaPase;
    }

    public String getFechaPase() {
        return fechaPase;
    }

    public void setSequencia(String sequencia) {
        this.sequencia = sequencia;
    }

    public String getSequencia() {
        return sequencia;
    }

    public void setVerifitem(String verifitem) {
        this.verifitem = verifitem;
    }

    public String getVerifitem() {
        return verifitem;
    }

    public void setCanal(String canal) {
        this.canal = canal;
    }

    public String getCanal() {
        return canal;
    }

    public void setFicha(String ficha) {
        this.ficha = ficha;
    }

    public String getFicha() {
        return ficha;
    }

    public void setFechaLevante(String fechaLevante) {
        this.fechaLevante = fechaLevante;
    }

    public String getFechaLevante() {
        return fechaLevante;
    }

    public void setItems(String items) {
        this.items = items;
    }

    public String getItems() {
        return items;
    }

    public void setPatron(String patron) {
        this.patron = patron;
    }

    public String getPatron() {
        return patron;
    }

    public void setKey_year(String key_year) {
        this.key_year = key_year;
    }

    public String getKey_year() {
        return key_year;
    }

    public void setKey_cuo(String key_cuo) {
        this.key_cuo = key_cuo;
    }

    public String getKey_cuo() {
        return key_cuo;
    }

    public void setKey_dec(String key_dec) {
        this.key_dec = key_dec;
    }

    public String getKey_dec() {
        return key_dec;
    }

    public void setKey_nber(String key_nber) {
        this.key_nber = key_nber;
    }

    public String getKey_nber() {
        return key_nber;
    }

    public void setSad_reg_year(String sad_reg_year) {
        this.sad_reg_year = sad_reg_year;
    }

    public String getSad_reg_year() {
        return sad_reg_year;
    }

    public void setSad_reg_serial(String sad_reg_serial) {
        this.sad_reg_serial = sad_reg_serial;
    }

    public String getSad_reg_serial() {
        return sad_reg_serial;
    }

    public void setSad_reg_nber(String sad_reg_nber) {
        this.sad_reg_nber = sad_reg_nber;
    }

    public String getSad_reg_nber() {
        return sad_reg_nber;
    }

    public void setSad_consignee(String sad_consignee) {
        this.sad_consignee = sad_consignee;
    }

    public String getSad_consignee() {
        return sad_consignee;
    }

    public void setSad_reg_date(String sad_reg_date) {
        this.sad_reg_date = sad_reg_date;
    }

    public String getSad_reg_date() {
        return sad_reg_date;
    }

    public void setSad_itm_total(String sad_itm_total) {
        this.sad_itm_total = sad_itm_total;
    }

    public String getSad_itm_total() {
        return sad_itm_total;
    }

    public void setParam(String param) {
        this.param = param;
    }

    public String getParam() {
        return param;
    }
}
