...
import anb.persistencia.RegistroControlDao;
...
public class RegistroControlNeg {

    private final RegistroControlDao dao = new RegistroControlDao();
    ...
    public Respuesta<Boolean> registra_control(RegistroControlForm bean){
        Respuesta<Boolean> respuesta = new Respuesta<Boolean>();
        respuesta.setCodigo(-1);
        respuesta.setResultado(false);
        if (estaConectadoBd()) {
            try {
                String result = dao.registra_control(bean);
                if (result == null) {
    ...
    public Respuesta<Boolean> verifica_registra_control(RegistroControlForm bean){
        Respuesta<Boolean> respuesta = new Respuesta<Boolean>();
        respuesta.setCodigo(-1);
        respuesta.setResultado(false);
        if (estaConectadoBd()) {
